...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Function to get typeDemandes and typeLicences data from PHP
    const typeDemandes = @json($typeDemandes);
    const typeLicences = @json($typeLicences);
    
    // Function to generate a new form HTML
    function generateNewForm(index) {
        let typeDemandesOptions = '<option value="">Select Type Demande</option>';
        typeDemandes.forEach(function(type) {
            typeDemandesOptions += `<option value="${type.id}">${type.nom_fr}</option>`;
        });
        
        let typeLicencesOptions = '<option value="">Select Type Licence</option>';
        typeLicences.forEach(function(type) {
            const name = type.nom || type.name;
            typeLicencesOptions += `<option value="${type.id}">${name}</option>`;
        });
    ...
    function reindexForms() {
        ...
        $('.checklist-form').each(function(newIndex) {
            ...
            $(this).find('input, textarea, select').each(function() {
                ...
                if (oldName) {
                    const newName = oldName.replace(/checklists\[\d+\]/, `checklists[${newIndex}]`);
        ...
        $('#checklistCount').text($('.checklist-form').length);
        formCount = $('.checklist-form').length;
    }
    
    // Add new checklist form
    $('#addChecklistBtn').on('click', function() {
        
        const newIndex = $('.checklist-form').length;
        const newForm = generateNewForm(newIndex);
        $('#checklistsContainer').append(newForm);
        $('#checklistCount').text(newIndex + 1);
    ...
    $(document).on('click', '.remove-form', function() {
        ...
        $(`.checklist-form[data-index="${index}"]`).remove();
        reindexForms();
    });
    
    // Remove last checklist
    $('#removeChecklistBtn').on('click', function() {
        
        const totalForms = $('.checklist-form').length;
        if (totalForms > 1) {
