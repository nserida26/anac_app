...
// SUPPRIM�: if ($currentDate->lte($typeExpiryDate)) {
...
// Grouper par codeAirCraft et garder la date d'expiration maximale
                            ...
                            @php
                                ...
                                if (in_array($demande->typeLicence->id, [36, 39])) {
                                    // Pour le type 36 et 39, on peut avoir plusieurs codes avec diff�rentes dates
                        ...
                        @if (!empty($qualification_amts) && $qualification_amts->isNotEmpty())
                            @foreach ($qualification_amts as $qualification_amt)
                                @php
                                    ...
                                    // SUPPRIM�: if ($currentDate->lte($amtExpiryDate)) {
                        ...
                        @if (!empty($qualification_atcs) && $qualification_atcs->isNotEmpty())
                            @foreach ($qualification_atcs as $qualification_atc)
                                @php
                                    ...
                                    // SUPPRIM�: if ($currentDate->lte($atcExpiryDate)) {
...
$instructeurString = implode('; ', $instructeurDetails);
                    ...
                    @if (!empty($medical_certificat))
                        @php
                            ...
                            // Date de base apr�s ajout des mois de validit�
                            ...
                            if ($licence->type_calcul == 'jours' && $licence->jours_supplementaires > 0) {
                                ...
                                // Prolonger jusqu'� la fin du mois
