    ...
    public function isAnnotedToDSF(): bool
{
    ...
    /**
     * Qui a rejeté la demande, d'après les indicateurs booléens du workflow
     * (etatDemande->dta_rejeter / dg_rejeter), et non la simple présence d'un motif.
     */
    ...
    public function getEtatWorkflowAttribute()
{
    ...
    /**
     * Accesseur pour typeVol (simule la relation pour la compatibilit�)
     */
    public function getTypeVolAttribute()
    {
        ...
        if ($this->type_demande_autorisation_id == 3 && $this->type_vol_ids) {
            ...
            if ($ids && !empty($ids)) {
                // Retourner le premier type de vol pour la compatibilit�
    ...
    /**
     * Accesseur pour les noms des types de vol (format�s)
     */
